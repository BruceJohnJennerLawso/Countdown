...
VectorVictor::Vector2::Vector2(variable ix, variable iy)
...
void VectorVictor::Vector2::Set_x(variable ix)
{	x = ix;					
}

void VectorVictor::Vector2::Set_y(variable iy)
{	y = iy;					
}

void VectorVictor::Vector2::Set_values(variable ix, variable iy)
{	x = ix;
	y = iy;
}
...
variable VectorVictor::Vector2::Get_x()
{	return x;
}

variable VectorVictor::Vector2::Get_y()
{	return y;
}
...
VectorVictor::Vector2 VectorVictor::Vector2::operator *= (const variable c)
...
{	VectorVictor::Vector2 Output;
	variable vector_magnitude = this->Get_vector_magnitude();
...
void VectorVictor::Vector2::Rotate_vector(variable rotation)
...
VectorVictor::Vector2 VectorVictor::Vector2::Get_rotated_vector(variable rotation)
...
variable VectorVictor::Vector2::Get_vector_magnitude()
{	variable output, vx, vy;
...
variable VectorVictor::Vector2::Get_vector_magnitude_squared()
{	variable output, vx, vy;
	vx = pow(x, 2);
	vy = pow(y, 2);
...
variable VectorVictor::Get_dot_product(variable x1, variable y1, variable x2, variable y2)
{	variable output = ((x1*x2)+(y1*y2));
	// not much to say here, just perform the standard dot-product operation
	// & return the result. same thing below
	return output;
}

variable VectorVictor::Get_dot_product(VectorVictor::Vector2 First_vector, VectorVictor::Vector2 Second_vector)
{	variable x1 = First_vector.Get_x();
	variable y1 = First_vector.Get_y();
	variable x2 = Second_vector.Get_x();
	variable y2 = Second_vector.Get_y();
	variable output = ((x1*x2)+(y1*y2));
...
variable VectorVictor::Get_vector_angle(VectorVictor::Vector2 First_vector, VectorVictor::Vector2 Second_vector)
{	// given that a dot b = ||a|| ||b|| cos(theta)
	...
	variable theta, cos_theta;
	variable dot_product, Magnitude1, Magnitude2;
...
variable VectorVictor::Get_cross_product(Vector2 First_vector, Vector2 Second_vector)
...
variable VectorVictor::Get_vector_separation(Vector2 First_vector, Vector2 Second_vector)
{	variable dx = (First_vector.x - Second_vector.x);
	variable dy = (First_vector.y - Second_vector.y);
	variable rad = sqrt((dx*dx)+(dy*dy));
